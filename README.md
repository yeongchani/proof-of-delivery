...
1. **고객이 대금을 맡깁니다.** 개발자와 합격 기준을 정하고, 계약에 대금을 예치합니다.
2. **납품물을 테스트합니다.** 개발자가 올린 코드를 미리 합의한 테스트로 검사합니다.
3. **결과를 검토하고 서명합니다.** 러너 운영자가 별도 환경에서 결과와 로그를 확인한 뒤 서명해 계약에 제출합니다.
4. **이의 기간이 지나면 지급합니다.** 이의가 없으면 누구든 `release()`를 호출해 개발자에게 대금을 지급할 수 있습니다.
